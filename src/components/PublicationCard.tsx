...
export default function PublicationCard({ pub, lang }: Props) {
  ...
  return (
    <article className="group relative rounded-2xl bg-gray-800/80 ring-1 ring-white/10 shadow
      ...
      <div className="p-6 space-y-3">
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          {pub.tags.map(tag => (
            <span key={tag}
              className="text-xs bg-gray-700 text-blue-100 px-2 py-1 rounded-md">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="pt-2 flex gap-3">
          ...
          {pub.links?.pdf && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 rounded-md bg-gray-700 px-3 py-1.5 text-sm font-medium text-blue-100
